...
- SQLite の接続は `check_same_thread=False` を指定して開かれ、
  Flask が複数スレッドから安全にデータベースへアクセスできます。
...
モンスターにはステータスの伸び方を決める「成長タイプ」が存在します。
従来の「平均型」「早熟型」「大器晩成型」に加え、「パワー型」「魔法型」「防御型」「スピード型」
を選択でき、魔法型はMPと魔力も上昇します。
各モンスターには `slime` や `dragon` などの種族を示す **family** フィールドも存在します。
ID単位のレシピが無くても、2体の family の組み合わせが `FAMILY_SYNTHESIS_RULES` に登録されていれば
合成が成立し、対応するモンスターが生成されます。
...
## 新機能
- アイテムは戦闘外で使用でき、HPを回復できます。戦闘中での使用は今後対応予定です。
